...
use scterm_core::{
    ...
    RingSize, Session, SessionPath, WindowSize,
};
use scterm_unix::{
    terminal_window_size, PtyCommand, RawModeGuard, SignalEvent, SignalWatcher, SocketTransport,
    UnixError, UnixSocketTransport,
};
...
const EXIT_STALE: i32 = 4;
const EXIT_SELF_ATTACH: i32 = 5;
const EXIT_NO_TTY: i32 = 6;
const EXIT_EXISTS: i32 = 7;
const EXIT_START_TIMEOUT: i32 = 8;
const DEFAULT_DETACH_CHAR: u8 = 0x1c;
...
fn parse_cli(argv: &[String]) -> Result<Action, CliError> {
    if argv.is_empty() {
...
fn default_child_command() -> Vec<String> {
    vec![
...
fn current_ancestry_chain(program: &str) -> Result<AncestryChain> {
    let env_name = session_env_var_name(program);
...
fn ensure_not_self_attach(program: &str, path: &SessionPath) -> Result<(), CliError> {
...
fn require_tty(program: &str) -> Result<(), CliError> {
...
fn current_session_path(program: &str) -> Result<SessionPath, CliError> {
    current_ancestry_chain(program)
        .map_err(runtime_error)?
        .innermost()
        .cloned()
...
fn resolve_session_path(program: &str, session: &str) -> Result<SessionPath, CliError> {
...
fn default_session_dir(program: &str) -> PathBuf {
...
fn app_log_root_for(path: &SessionPath) -> PathBuf {
    path.as_path()
        .parent()
...
fn session_label(path: &SessionPath) -> String {
    path.file_name()
...
fn session_state(path: &SessionPath) -> Result<SessionState, CliError> {
    let transport = UnixSocketTransport;
    match transport.connect(path) {
...
fn current_window_size(fd: &impl AsFd) -> WindowSize {
    terminal_window_size(fd).unwrap_or_else(|_| WindowSize::new(24, 80, 0, 0))
}

fn domain_error(error: impl Into<anyhow::Error>) -> CliError {
...
fn unix_error(error: UnixError) -> CliError {
    match error {
